    ...
    <section className="bg-white border-y border-[#E7E7E7]" data-testid="mini-trust-strip">
      <div className="container-page py-5 sm:py-7 lg:py-8">
        <div className="flex flex-col lg:flex-row lg:items-center gap-4 sm:gap-5 lg:gap-8">
          <div className="inline-flex items-center gap-2.5 rounded-full bg-[#F7F7F5] border border-[#E7E7E7] px-3 sm:px-4 py-2 shrink-0 self-start" data-testid="live-counter">
            ...
            <span className="text-[12px] sm:text-[13px] text-[#111111] font-semibold">
              Сьогодні вже <span className="font-heading">{count}</span> підборів
            </span>
          </div>

          <ul className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-5 flex-1 min-w-0">
            {POINTS.map((p, i) => (
              <li key={i} className="flex items-start gap-2.5 sm:gap-3 min-w-0">
                <span className="shrink-0 h-8 w-8 sm:h-9 sm:w-9 rounded-[10px] bg-[#F1F1EF] border border-[#E7E7E7] grid place-items-center">
                  <p.icon className="h-4 w-4 text-[#111111]" strokeWidth={1.75} />
                </span>
                <span className="text-[12.5px] sm:text-[14px] text-[#1A1A1A] leading-snug font-medium min-w-0">
                  {p.text}
                </span>
              </li>
            ))}
