  ...
  return (
    <section className="section-y bg-[#111111] text-white" data-testid="how-it-works-section">
      <div className="container-page">
        ...
        <div className="mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5" data-testid="how-we-work-steps">
          {STEPS.map((s, i) => (
            <div key={s.title} className="relative rounded-[24px] border border-[#2A2A2A] bg-[#1A1A1A] p-7 hover:border-[#3A3A3A] transition-colors">
              <div className="flex items-start justify-between">
                <div className="h-12 w-12 rounded-[14px] bg-white text-[#111111] grid place-items-center">
                  <s.icon className="h-5 w-5" strokeWidth={1.75} />
                </div>
                <span className="font-heading text-4xl font-semibold text-[#2A2A2A] leading-none tracking-tight">0{i + 1}</span>
              </div>
              <h3 className="mt-8 font-heading font-semibold text-[19px] text-white tracking-tight">{s.title}</h3>
              <p className="mt-2 text-sm text-[#A1A1A1] leading-relaxed">{s.desc}</p>
            </div>
          ))}
        </div>
      </div>
